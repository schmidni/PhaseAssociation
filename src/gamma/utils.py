...
def to_seconds(t): return t.timestamp(tz="UTC")


def from_seconds(t): return pd.Timestamp.utcfromtimestamp(
...
def convert_picks_csv(picks, stations, config):
    ...
    pick_station_id = picks.apply(
        lambda x: x.id + "_" + x.type, axis=1).to_numpy()
...
def associate(
    ...
    centers_init = init_centers(
        config, data_, locs_, time_range, max_num_event)
    ...
    if "covariance_prior" in config:
        ...
        x_std = np.sqrt(np.average(
            (locs_[:, 0] - x_mean) ** 2, weights=weight))
        y_std = np.sqrt(np.average(
            (locs_[:, 1] - y_mean) ** 2, weights=weight))
    ...
    for i in range(len(centers_init)):
        ...
        if config["use_amplitude"]:
            ...
            if "max_sigma12" in config:
                idx_cov = np.abs(gmm.covariances_[
                                 i, 0, 1]) < config["max_sigma12"]
...
def init_centers(config, data_, locs_, time_range, max_num_event=1):
    ...
    index = np.argsort(data_[:, 0])[:: max(
        len(data_) // num_t_init, 1)][:num_t_init]
    ...
    z_init = np.linspace(
        config["z(km)"][0], config["z(km)"][1], initial_points[2] + 2)[1:-1]
